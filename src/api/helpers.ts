...
export const buildFunctionalBMMap = (
  ...
  grahaNatures.forEach((row) => {
    ...
    const nItems = nSet instanceof Array ? nSet : [];
    nItems.push(row.key);

    mp.set(row.nature, nItems);
  });
  return mp;
};
...
export const applyDashaTransitClasses = (
  transitKeys: string[],
  body: Graha,
  cls = []
) => {
  if (transitKeys.length > 0) {
    const levels = transitKeys
      .map((key, index) => {
        return { key, index };
      })
      .filter((ki) => ki.key === body.key)
      .map((ki) => ki.index + 1);
    if (levels.length > 0) {
      levels.forEach((lv, i) => {
        cls.push(["dasha", i < 1 ? "level" : "sublevel", lv].join("-"));
      });
    }
  }
};
