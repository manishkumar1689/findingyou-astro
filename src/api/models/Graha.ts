...
import mrityubhagaData from "../mappings/mrityubhaga-data";
import {
  calcVargaValue,
  nakshatra28Progress,
  withinNakshatra28,
...
export class Graha {
  ...
  getMrityubhaga(range = "standard") {
    const { standard, alternative } = mrityubhagaData;
    const rangeSet = range === "alt" ? alternative : standard;
    let refKey = this.key;
    switch (this.key) {
      case "md":
      case "gu":
    ...
    const row = rangeSet.values.find((r) => r.graha === refKey);
    let degree = -180;
    const signIndex = this.sign - 1;
    if (row) {
      if (signIndex < row.degrees.length) {
        degree = row.degrees[signIndex];
      }
    }
    return degree + signIndex * 30;
  }

  getMrityubhagaDiff(range = "standard") {
    const { orb } = mrityubhagaData;
    const diff = this.getMrityubhaga(range) - this.longitude;
    const min = 0 - orb;
    const withinRange = diff <= orb && diff >= min;
    return withinRange ? diff : -360;
  }
